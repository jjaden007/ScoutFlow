  ...
  if (req.method === 'POST') {
    ...
    if (pass === '********') {
      const { data: existing } = await supabaseAdmin
        .from('user_smtp_settings').select('pass').eq('user_id', authUser.id).single();
      finalPass = existing?.pass || '';
    }
    const { error } = await supabaseAdmin
      .from('user_smtp_settings')
      .upsert(
        { user_id: authUser.id, host, port, user, pass: finalPass, from_email, secure: secure ? 1 : 0, updated_at: new Date().toISOString() },
        { onConflict: 'user_id' }
      );
