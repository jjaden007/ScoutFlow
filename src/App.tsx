...
import { getLeads, saveLead, updateLead, deleteLead, getProfile, updateProfile, sendEmail, Lead, UserProfile, signup, login, getMe, logout, createCheckoutSession, User as UserType, getSmtpSettings, updateSmtpSettings, SmtpSettings, getGoogleStatus, getGoogleAuthUrl, getGoogleLoginUrl, disconnectGoogle } from './services/api';
...
const AuthPage = ({ mode, onSwitch, onSuccess }: { mode: 'login' | 'signup', onSwitch: () => void, onSuccess: (user: UserType) => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      console.log('Attempting signup/login...', { email, mode });
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log('Opening Google login popup:', url);
      ...
      >
        ...
        <div className="mt-6">
          ...
          <button 
            ...
            disabled={loading}
            className="w-full bg-white border border-slate-200 text-slate-700 font-bold py-4 rounded-2xl transition-all shadow-sm hover:bg-slate-50 flex items-center justify-center gap-3 disabled:opacity-50"
          >
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'prospector' | 'leads' | 'profile' | 'settings'>('dashboard');
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        if (originUrl.hostname !== currentUrl.hostname && !event.origin.includes('localhost')) {
          return;
        }
      } catch (e) {
        return;
      ...
      if (event.data?.type === 'GOOGLE_LOGIN_SUCCESS') {
        console.log('Received GOOGLE_LOGIN_SUCCESS', event.data.user);
  ...
  const handleAuthSuccess = (user: UserType) => {
    ...
    setUser(user);
    if (user.is_paid) {
      console.log('User is paid, redirecting to dashboard');
      setView('dashboard');
      fetchLeads();
      fetchProfile();
      fetchSmtpSettings();
      ...
      console.log('User is not paid, redirecting to pricing');
      setView('pricing');
    }
  };
  ...
  const handleSaveSmtp = async (e: React.FormEvent) => {
    ...
    try {
      await updateSmtpSettings(newSettings);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'settings' && (
            ...
            >
              <header className="mb-10">
                <h1 className="text-4xl font-bold text-slate-900 tracking-tight mb-2">System Settings</h1>
                <p className="text-slate-500 font-medium">Configure your SMTP and integration preferences.</p>
              </header>
              ...
              <div className="bg-white border border-slate-200 rounded-[2rem] p-10 shadow-sm">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">Email Integrations</h2>
                    <p className="text-xs text-slate-400 font-medium">Choose how you want to send outreach emails.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-3xl p-8 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-100 rounded-full -mr-12 -mt-12 blur-2xl opacity-50" />
                    <div className="flex items-center gap-4 mb-6 relative z-10">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900">Google / Gmail</h3>
                        <p className="text-xs text-slate-500 font-medium">Send via Gmail API</p>
                      </div>
                    </div>
                    
                    {googleStatus.connected ? (
                      <div className="space-y-4 relative z-10">
                        <div className="flex items-center gap-2 text-emerald-600 bg-emerald-50 px-3 py-2 rounded-xl text-xs font-bold w-fit">
                          <CheckCircle2 size={14} /> Connected as {googleStatus.email}
                        </div>
                        <button 
                      ...
                      <button 
                        ...
                        className="w-full bg-white border border-slate-200 text-slate-700 py-3 rounded-2xl font-bold text-sm shadow-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2 relative z-10"
                      >
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-3xl p-8">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900">Custom SMTP</h3>
                        <p className="text-xs text-slate-500 font-medium">Use your own mail server</p>
                      </div>
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed mb-4">
                      Configure custom SMTP if you prefer to use a specialized email provider like SendGrid, Mailgun, or your own private server.
                    </p>
                    <div className="flex items-center gap-2 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                      <Clock size={12} /> Configurable below
                    </div>
                  </div>
                </div>

                <div className="pt-8 border-t border-slate-100">
                  <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest mb-8">SMTP Configuration</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">SMTP User</label>
                      <input 
                        name="user"
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        name="from_email"
                        defaultValue={smtpSettings?.from_email}
                        placeholder="Agency Name <hello@agency.com>"
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl py-3 px-4 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm"
                      />
                    </div>
                    <div className="flex items-center gap-3 pt-8">
                      <input 
                        ...
                        name="secure"
                        id="secure"
                        defaultChecked={smtpSettings?.secure}
                        className="w-5 h-5 rounded border-slate-200 text-indigo-600 focus:ring-indigo-500"
                      />
                      <label htmlFor="secure" className="text-sm font-bold text-slate-600">Use SSL/TLS (Port 465)</label>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 px-10 rounded-2xl transition-all shadow-xl shadow-indigo-600/20 flex items-center gap-2 disabled:opacity-50"
                    >
                      {isSavingSmtp ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
